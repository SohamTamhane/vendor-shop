    ...
    function addToCartFunc(){
        if(loginInfo.status){
            ...
            if(!existingCart){
                ...
                setCart([...cart, {...product.data[0], qty, orderid, status, user: loginInfo.email}]);
                navigate('/cart')
            }
        }
        else{
            ...
            {product?.data?.map((elm)=>(    
                <div key={elm?.id} className="single-product-box">
                    <div className="single-product-left-div">
                        <img className="single-product-img" src={process.env.REACT_APP_FILE_URL+elm?.image[0]} alt="product-img" />
                    </div>
